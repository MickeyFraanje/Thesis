# -*- coding: utf-8 -*-
"""
Created on Tue May  5 21:23:06 2020

@author: Mickey

Script that creates the sentiment dataset used in the study
"""
...
for week in range (20):
    ...
    for i in range(7):
        ...
        def sentiment_parser():
            # Extracts the sentiment data from GDELT and creates a dataframe
            s = merged_data["V2Tone"].str.split(',', expand = True)
            s.columns = ["Tone", "Positive Score", "Negative Score", 
                         "Polarity", "Activity Reference Density", 
                         "Self/Group Reference Density", "Word Count"]
            s[["Tone", "Positive Score", "Negative Score", "Polarity", 
               "Activity Reference Density", "Self/Group Reference Density", 
               "Word Count"]] = s[["Tone", "Positive Score", "Negative Score", 
                                   "Polarity", "Activity Reference Density", 
                                   "Self/Group Reference Density", 
                                   "Word Count"]].astype(float)
            return s
        
        sentiment = sentiment_parser().fillna(0)
        
        # Creates a single dataset
        fd = merged_data[["Country", "CountryCode"]].join(sentiment, 
                                                          how='outer')
